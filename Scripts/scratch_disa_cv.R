...
file <- "~/GitHub/Enhanced_Monitoring/Data/Disa_new/monthly/Relatorio Mensal de Carga Viral Maio 2024.xlsx"
month_input <- "2024-05-20"
  ...
  df_1 <- df %>%
    dplyr::mutate(periodo = as.Date(month, "%Y-%m-%d"),
                  ...
                  disagregacao = dplyr::case_when(disagregacao == "routine" ~ "Rotina",
                                                  disagregacao == "failure" ~ "Falência Terapéutica",
                                                  disagregacao == "repeat" ~ "Pós-Amamentação",
                                                  disagregacao == "unspecified" ~ "Não Especificado",
                                                  str_detect(disagregacao_sub, "s1.") ~ "P1: Recolha à Recepção",
                                                  str_detect(disagregacao_sub, "s2.") ~ "P2: Recepção à Registo",
                                                  str_detect(disagregacao_sub, "s3.") ~ "P3: Registo à Análise",
                                                  str_detect(disagregacao_sub, "s4.") ~ "P4: Análise à Validação",
                                                  disagregacao == "" ~ NA_character_),
                  
                  resultado_estado = dplyr::case_when(resultado_estado == "suppress" ~ "Suprimida",
                                                      resultado_estado == "unsuppress" ~ "Não Suprimida"),
                  
                  disagregacao_sub = disa_uid,
                  
                  VL = dplyr::case_when(resultado_estado == "Suprimida" | resultado_estado == "Não Suprimida" & indicador == "vl" ~ valor),
...
df <- reshape_disa_cv(file, month = month_input)
...
df |> count(wt = VL)
df1 |> filter(indicador == "VL") |> count(wt = valor)

df |> count(wt = VLS)
df1 |> filter(indicador == "VLS") |> count(wt = valor)

df |> count(wt = TAT)
df1 |> filter(indicador == "TAT") |> count(wt = valor)


df1 <- df |> 
  select(!c(starts_with("his_"), site_nid, snu, psnu, sitename)) |> 
  mutate(fonte = "DISA",
         periodo_coorte = NA_character_,
         resultado_estado = NA_character_,
